...
function dkaController($scope,$interval,$sce,$http,$timeout){
	...
	// I need to use a then
	$scope.actRandomly = function() {
		$http({
			method: 'GET',
			url: $scope.configfile.kbserverip+'bot/isbusy'// url should be loaded from a conf file
		}).then(function successCallback(response) {
			if(response.data == "false") {
				var randomQuery = $scope.buildRandomQuery();
				//console.log("Random query selected: " + randomQuery)
	...
	function placeRobot() {
		$http({
		    method: 'GET',
		    url: $scope.configfile.kbserverip+'bot/wru', // url should be loaded from a conf file
	...
	function updateRooms(){
		...
		$http({
			method: 'GET',
			url: $scope.configfile.kbserverip+'query', // url should be loaded from a conf file
	...
	{ 	'name':'Query 4',
		'value':'select ?room ?temperature where {graph ?expiryDateInMs { VALUES(?room) { ( <http://data.open.ac.uk/kmi/location/Room20> ) (<http://data.open.ac.uk/kmi/location/Room22>) } ?room <http://data.open.ac.uk/kmi/robo/hasTemperature> ?temperature. } }'
	}];
	// $scope.queries = [
// 		{'name':'Query 1',
...
// 		}];
	...
	$scope.getPlan = function() {
		$http({
			method: 'GET',
			url: $scope.configfile.kbserverip+'bot/currentplan' // url should be loaded from a conf file
		}).then(function successCallback(response) {
	...
	$scope.updatePlanExecution = function(){ 
		...
		$http({
			method: 'GET',
			url: $scope.configfile.kbserverip+'bot/doing', // url should be loaded from a conf file
			headers: {'Content-Type': 'application/x-www-form-urlencoded'}
		}).then(function successCallback(response) {
			if (response.data == "") {
	...
	$scope.abort = function() {
		$http({
			method: 'DELETE',
			url: $scope.configfile.kbserverip+'bot/abort', // url should be loaded from a conf file
			//headers: {'Content-Type': 'application/x-www-form-urlencoded'}
		}).then(function successCallback(response) {
	...
	$scope.executePlanForQuery = function(query) {
		$http({
			method: 'GET',
			url: $scope.configfile.kbserverip+'planner/plan', // url should be loaded from a conf file
			headers: {'Content-Type': 'application/x-www-form-urlencoded'},
			params: {query: query}
		}).then(function successCallback(response) {
			if($scope.createPlan(response.data)) {
	...
	$scope.sendPlan = function(queryForPlan) {
			$http({
				method: 'GET',
				url: $scope.configfile.kbserverip+'bot/send', // url should be loaded from a conf file
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
